...
当前处于：**第三个月第 9 周，第 1～33 课均已完成、验收并合并；下一步第 34 课区分深度测试与深度写入。**
...
下一步：从稳定的最新 `main` 开始第 34 课，以现有两个矩形完成深度测试与深度写入的受控对照。使用 RenderCommand 封装 `glDepthMask`，Renderer 负责每帧清屏前恢复写入和逐物体切换；保持现有资源所有权与单向依赖，不新增资源类或依赖。
...
| 第 9 周 | 深度测试 | 第 33 课已完成并合并；下一步第 34 课区分深度测试与写入 |
