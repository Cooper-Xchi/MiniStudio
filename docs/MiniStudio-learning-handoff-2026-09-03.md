...
当前处于：**第 4 周第 14 课已完成验收，等待提交、推送并合并；之后进入第 15 课。**
...
第 14 课已在 `codex/lesson-14-raii-move-only` 完成验收：`ShaderProgram` 与 `VertexArray` 保持独占 OpenGL 资源所有权，禁止复制并实现 `noexcept` move 构造和 move 赋值；`Renderer` 显式遵循相同的 move-only 规则，`GlfwWindow` 因当前 Context 与 GLFW 全局生命周期约束继续保持不可移动。资源接管前会释放目标对象原有资源，接管后会清空源对象句柄，`Release()` 保持私有并将状态复位。普通与 Sanitizer 构建无警告；临时验证程序通过类型特征检查，并让真实 OpenGL Program、VAO 和 VBO 经历 move 构造、self-move 与 move 赋值，三次均成功绘制三角形且 ASan/UBSan 无报错。学习者能够解释 move 是资源句柄所有权转移而不是复制或重建 GPU 资源。本课尚未提交、推送或合并；完成版本操作后，下一步从最新 `main` 创建第 15 课分支，学习 Debug/Release、OpenGL 错误与故障注入。
...
| 第 4 周 | 最小 RAII 封装、Debug/Release、故障定位、README 与生命周期说明 | 进行中；第 14 课已验收，等待提交、推送并合并 |
