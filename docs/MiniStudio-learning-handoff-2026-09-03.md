...
当前处于：**第三个月第 9 周，第 1～33 课均已完成、验收并合并；第 34 课已通过代码、GPU 对照与理解验收，待确认提交、推送并合并。**
...
本课清除边界：`glClear` 受深度写入掩码影响，因此每帧必须先恢复深度写入再清颜色/深度，然后应用各矩形的实验配置；一帧结束时恢复允许写入。最终保留两个矩形都允许深度写入的正常配置。验收需有三组运行对照、无警告/无 OpenGL 错误的构建运行结果，并能解释关闭写入与关闭测试的不同，以及恢复写入为什么必须发生在清屏之前。练习已完成，最终验证与配置见下文；提交、推送和合并仍待学习者确认。
...
下一步：等待学习者明确确认是否提交第 34 课、推送课程分支并合并回 `main`，保留课程分支。合并并确认稳定基线后再安排第 35 课。
...
| 第 9 周 | 深度测试 | 第 33 课已完成并合并；第 34 课已验收，待确认提交、推送与合并 |
