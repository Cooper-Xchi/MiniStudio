...
项目第 1～33 课均已通过验收并合并。第 33 课复用顶点和纹理资源绘制两个旋转矩形，以逐帧深度清除和 `GL_LESS` 保证近处遮挡不受绘制顺序影响；下一课区分深度测试与深度写入。
...
前三十三课均已完成并合并回 `main`。第 33 课课程分支 `codex/lesson-33-depth-testing` 已推送并继续保留，稳定的 `main` 已包含深度测试及验收记录。
