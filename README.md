...
项目已经完成第 1～21 课，相关成果均已通过验收并合并。项目使用固定版本的 `stb_image` 同步解码 PNG，CPU 像素由 `ImageData` 的 `std::vector` 管理，再由主线程上传到 `Texture2D`；下一步统一图片行方向与纹理 UV 的约定。
...
前二十一课均已完成并合并回 `main`。第二十一课的课程分支为 `codex/lesson-21-stb-image-loading`，该分支已推送并继续保留；当前稳定的 `main` 已包含同步外部图片加载链路。
