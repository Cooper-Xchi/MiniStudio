...
项目第 1～32 课均已通过验收并合并。第 33 课深度测试已通过代码、运行与理解验收：复用顶点和纹理资源绘制两个旋转矩形，以逐帧深度清除和 `GL_LESS` 保证近处遮挡不受绘制顺序影响；当前成果保留在课程分支，待确认提交、推送并合并。
...
前三十二课均已完成并合并回 `main`。第 33 课在 `codex/lesson-33-depth-testing` 已验收，尚未提交、推送或合并；稳定的 `main` 当前仍以第 32 课为基线。
