...
项目已经完成第 1～23 课，相关成果均已通过验收并合并。第 24 课已在课程分支通过验收：`Texture2D` 会检查自身初始化调用产生的 OpenGL 错误，失败时回滚为可安全析构和重新初始化的空状态；当前等待提交并合并。
...
前二十三课均已完成并合并回 `main`。第二十四课在 `codex/lesson-24-texture-error-boundary` 完成验收，尚未提交或合并；当前稳定的 `main` 仍停留在第二十三课。
