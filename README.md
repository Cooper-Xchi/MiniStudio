...
项目已经完成第 1～21 课，相关成果均已通过验收并合并。第 22 课已在课程分支通过验收：图片解码后显式交换上下像素行，统一为适合当前 OpenGL UV 的底行优先 CPU 数据；当前等待提交并合并。
...
前二十一课均已完成并合并回 `main`。第二十二课在 `codex/lesson-22-texture-orientation` 完成验收，尚未提交或合并；当前稳定的 `main` 仍停留在第二十一课。
