...
项目第 1～33 课均已通过验收并合并。第 34 课已通过代码、GPU 对照与理解验收：独立控制每个矩形的深度写入，区分深度比较、颜色输出与深度更新，并在清屏前和帧末恢复允许写入；成果保留在课程分支，待确认提交、推送并合并。
...
前三十三课均已完成并合并回 `main`。第 34 课在 `codex/lesson-34-depth-write-mask` 已验收，尚未提交、推送或合并；稳定的 `main` 当前仍以第 33 课为基线。
