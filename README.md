...
项目已经完成第 1～26 课，相关成果均已通过验收并合并。第 27 课已在课程分支通过验收：顶点 Shader 使用独立 view 矩阵，把相机世界位置转换为反向的世界位移；当前等待提交并合并。
...
前二十六课均已完成并合并回 `main`。第二十七课在 `codex/lesson-27-view-matrix` 完成验收，尚未提交或合并；当前稳定的 `main` 仍停留在第二十六课。
