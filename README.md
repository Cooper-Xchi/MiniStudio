...
项目已经完成第 1～22 课，相关成果均已通过验收并合并。第 23 课已在课程分支通过验收：外部纹理会从 0 级像素生成完整 Mipmap，并使用三线性缩小过滤降低高频纹理的噪点；当前等待提交并合并。
...
前二十二课均已完成并合并回 `main`。第二十三课在 `codex/lesson-23-texture-mipmaps` 完成验收，尚未提交或合并；当前稳定的 `main` 仍停留在第二十二课。
