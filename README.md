...
项目已经完成第 1～22 课，相关成果均已通过验收并合并。图片解码后会显式交换上下像素行，统一为适合当前 OpenGL UV 的底行优先 CPU 数据；下一步为外部纹理生成 Mipmap，并观察缩小采样。
...
前二十二课均已完成并合并回 `main`。第二十二课的课程分支为 `codex/lesson-22-texture-orientation`，该分支已推送并继续保留；当前稳定的 `main` 已包含统一的图片行方向约定。
