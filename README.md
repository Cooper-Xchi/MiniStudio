...
项目已经完成第 1～25 课，相关成果均已通过验收并合并。第 26 课已在课程分支通过验收：Application 使用单调时钟计算累计秒数，Renderer 据此每帧更新模型旋转，旋转速度不依赖帧率；当前等待提交并合并。
...
前二十五课均已完成并合并回 `main`。第二十六课在 `codex/lesson-26-frame-time-rotation` 完成验收，尚未提交或合并；当前稳定的 `main` 仍停留在第二十五课。
