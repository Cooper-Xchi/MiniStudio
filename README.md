...
项目第 1～48 课均已通过验收并合并。第 49 课「Alpha 裁剪与带孔平面」已完成并通过验收，等待提交合并。当前依赖形成 `Application → GlfwWindow/Camera/Renderer → RenderCommand → OpenGL`；窗口提供值类型输入快照并维护鼠标采样基准，Application 决定输入映射，Camera 保持纯 CPU 职责。
...
- 完成第四十五课的鼠标位移与焦点切换：GlfwWindow 按值返回鼠标 delta 与按钮状态，在捕获、释放和焦点切换时维护采样基准；Application 用左键捕获、右键释放并打印非零样本，尚未驱动相机旋转。
- 完成第四十六课的 yaw／pitch 观察方向：Camera 保存角度并推导归一化 forward，pitch 限制在正负 89°；Application 以独立灵敏度把鼠标像素位移映射为角度，不重复乘帧时间。
- 完成第四十七课的相机局部移动与斜向归一化：Camera 按值提供 forward/right，Application 在本帧旋转后组合 W/A/S/D 局部方向，非零时归一化后再按速度和帧间秒数移动。
- 完成第四十八课的交互相机回归与架构复盘：在公司 macOS arm64 实测移动、转头、焦点、resize、最小化恢复和关闭路径，并记录模块职责、所有权、主线程执行与 GPU 资源先于 Context 释放的析构顺序。
...
前 48 课均已完成并合并回 `main`。第 49 课已在 `codex/lesson-49-alpha-cutout` 完成并通过验收，等待提交、推送和合并；第 50 课尚未启动。
...
完整的阶段目标、验收标准和求职时间线见[课程路线](docs/MiniStudio-curriculum-24-36-months.md)。每节课的讲解内容、核心练习、边界和验收标准见[逐课教案索引](docs/lessons/README.md)，第 37～52 课另有详细步骤、易错点和追问，其中第 37～48 课已合并、第 49 课已验收并等待合并、第 50～52 课待执行。24 个月是核心路线，25～36 个月是进阶路线；课程不能替代岗位要求的商业项目年限。
