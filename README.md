...
项目第 1～34 课均已通过验收并合并。第 34 课独立控制每个矩形的深度写入，区分深度比较、颜色输出与深度更新，并在清屏前和帧末恢复允许写入；最终保留两个矩形都写深度的正常配置。
...
前三十四课均已完成并合并回 `main`。第 34 课课程分支 `codex/lesson-34-depth-write-mask` 已推送并继续保留，稳定的 `main` 已包含深度写入控制及验收记录。
