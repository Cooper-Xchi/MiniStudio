...
项目已经完成第 1～25 课，相关成果均已通过验收并合并。项目接入 GLM，通过 `model` uniform 把 CPU 端的平移与缩放矩阵上传到顶点 Shader；下一步建立累计帧时间与逐帧旋转链路。
...
前二十五课均已完成并合并回 `main`。第二十五课的课程分支为 `codex/lesson-25-model-matrix`，该分支已推送并继续保留；当前稳定的 `main` 已包含 GLM 模型矩阵链路。
